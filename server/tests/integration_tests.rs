...
mod integration_tests {
    ...
    #[test]
    fn test_not_tags_filtering() {
        if !should_run_integration_tests() {
            eprintln!(
                "⏭️  SKIPPED: test_not_tags_filtering (set RUN_INTEGRATION_TESTS=1 to enable)"
            );
            return;
        }
        ...
        let all_jukebox_songs = mpd_conn.mpd.search(&query, None).expect("Failed to search");
        ...
        let filtered_songs = tags_data.get_allowed_songs(&mut mpd_conn);
        ...
        println!("✓ Not tags filtering test passed");
    }
    ...
    fn test_unicode_handling() {
        ...
        let search_results = mpd_conn.mpd.search(&query, None).expect("Failed to search");
        ...
        println!("✓ Unicode handling test passed");
    }
    ...
    fn test_long_names_handling() {
        if !should_run_integration_tests() {
            eprintln!(
                "⏭️  SKIPPED: test_long_names_handling (set RUN_INTEGRATION_TESTS=1 to enable)"
            );
            return;
        }
        ...
        println!("✓ Long names handling test passed");
    }
    ...
    fn test_minimal_metadata_handling() {
        if !should_run_integration_tests() {
            eprintln!(
                "⏭️  SKIPPED: test_minimal_metadata_handling (set RUN_INTEGRATION_TESTS=1 to enable)"
            );
            return;
        }
        ...
        println!("✓ Minimal metadata handling test passed");
    }
}
