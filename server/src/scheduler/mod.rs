...
async fn scheduler_mainbody(app_state: AppState) {
    ...
    loop {
        ...
        if scheduler_cycle % 20 == 0 { // Every minute (3s * 20)
        ...
        if locked_song_queue.len() == 0 {
            ...
            locked_song_queue.shuffle_and_add_with_cache_async(
                &*locked_tags_data, 
                &mut *locked_mpd_conn
            ).await;
        }
        ...
        if scheduler_cycle % 100 == 0 { // Every 5 minutes
            ...
            println!("[+] Cache stats - Hits: {}, Misses: {}, Hit rate: {:.1}%", hits, misses, hit_rate);
            println!("[+] Cache validity - Regular: {}, Album: {}", regular_valid, album_valid);
        }
...
async fn background_precompute_task(app_state: AppState) {
    ...
    loop {
        ...
        locked_song_queue.background_precompute(
            &*locked_tags_data,
            &mut *locked_mpd_conn
        ).await;
        
        let precompute_time = precompute_start.elapsed();
        if precompute_time.as_millis() > 100 { // Only log if it took significant time
            println!("[+] Background precompute completed in {:?}", precompute_time);
        }
        ...
        let album_aware = *app_state.album_aware.read().await;
        if album_aware && cycle % 2 == 0 { // Extra precompute every minute for album mode
            tokio::time::sleep(Duration::from_secs(30)).await;
